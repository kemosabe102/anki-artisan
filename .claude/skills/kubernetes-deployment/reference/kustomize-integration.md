...
**Purpose**: Kustomize workflow patterns for deployment-release agent

**Audience**: deployment-release agent (reference during manifest management and deployments)
...
```bash
AGENT_NAME=deployment-release kubectl kustomize k8s/local/
```
...
```bash
AGENT_NAME=deployment-release kubectl apply -k k8s/local/
```
...
```bash
AGENT_NAME=deployment-release kubectl diff -k k8s/local/
```
...
```bash
AGENT_NAME=deployment-release kubectl delete -k k8s/local/
```
...
- `.claude/agents/deployment-release.md` - Complete deployment workflow
- `docs/04-guides/deployment-release/manifest-editing-protocol.md` - Manifest editing guide
