...
1. **Deploy observability infrastructure** (Jaeger pods, OTEL Collector, Prometheus) - deployment-release domain
2. **Deploy telemetrygen Jobs** for load testing infrastructure - deployment-release domain
3. **Instrument application code** (OpenTelemetry SDK in Python/Go) - Delegate to development (NOT deployment-release)
...
AGENT_NAME=deployment-release telemetrygen traces --otlp-endpoint otel-collector-service.gauntlet-agents.svc.cluster.local:4317 --otlp-insecure --duration 5s
...
AGENT_NAME=deployment-release kubectl apply -f k8s/local/telemetrygen-job.yaml

# Monitor job completion
AGENT_NAME=deployment-release kubectl logs -n gauntlet-agents -l job-name=telemetrygen-load-test -f
...
```bash
AGENT_NAME=deployment-release telemetrygen traces --otlp-insecure --duration 5s
```
...
- ❌ Do NOT use telemetrygen for application code instrumentation (delegate to development)
