...
→ Result: FLAKY test quarantined, delegate to code-quality with indicator context
```
...
### Integration 1: code-quality Agent

**Relationship**: code-quality detects flaky tests and delegates redesign to code-quality

**Coordination Pattern**:

- code-quality runs tests → detects inconsistent failures → validates flakiness
- code-quality scans test code for flakiness indicators
- code-quality delegates to code-quality with indicators and recommendations
- code-quality redesigns test for reliability (better isolation, mocking, synchronization)
- code-quality returns improved test → code-quality validates with N-run

**Example Usage**:

```python
# code-quality flakiness detection and delegation
...
if 0.15 < failure_rate < 1.0:
    ...
    # Delegate to code-quality with context
...
- code-quality depends on accurate flakiness detection
...
**Relationship**: code-quality provides flakiness metrics for CI/CD quality gates

**Coordination Pattern**:

- CI executes test suite via code-quality
- code-quality tracks retry patterns and failure rates
- code-quality reports flakiness metrics to CI
...
- CI depends on code-quality's flakiness detection accuracy
- code-quality depends on CI's execution environment consistency
