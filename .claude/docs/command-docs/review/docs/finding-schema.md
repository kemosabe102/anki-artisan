...
Complete structure for review findings.
...
| Field | Type | Description |
|-------|------|-------------|
| `confidence_sources` | array | List of sources that validated the finding |
| `investigation_trail` | object | Research history (initial, context7, perplexity, final) |
...
| `cvss_score` | float | CVSS severity score (security findings) |
...
|----------|-------------------|------------------------|
| Critical | 0.90 | ALWAYS Context7 + Perplexity |
...
| Severity | Report Section | Max Allowed |
|----------|---------------|-------------|
| Critical | CRITICAL (must fix) | 0-3 |
| High | HIGH (should fix) | 0-5 |
| Medium | MEDIUM (consider) | 0-5 |
| Low | LOW (optional) | 0-5 |
| Nit | NITS (informational) | 0-2 |
