...
<Steps>
  ...
  <Step title="Save and use">
    ...
    ```
    > Use the code-quality subagent to check my recent changes
    ```
...
Plugins (see `plugins.md`) can provide custom subagents that integrate seamlessly with Claude Code. Plugin agents work identically to user-defined agents and appear in the `/agents` interface.
...
- Can be invoked explicitly: "Use the code-quality agent from the security-plugin"
- Can be invoked automatically by Claude when appropriate
- Can be managed (viewed, inspected) through `/agents` interface

See the plugin documentation for details on creating plugin agents.
...
Subagents can be granted access to any of Claude Code's internal tools. See the `settings.md` for a complete list of available tools.
...
> Have the code-quality subagent look at my recent changes
...
- `plugins.md` - Extend Claude Code with custom agents through plugins
- `slash-commands.md` - Learn about other built-in commands
- `settings.md` - Configure Claude Code behavior
- `hooks.md` - Automate workflows with event handlers