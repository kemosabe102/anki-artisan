...
**Inherited Behavior**:
- Output schema structure (SUCCESS/FAILURE variants)
- Confidence thresholds (0.70 finding, 0.50 pair inclusion)
- Error recovery patterns

---

# Integration Boundary Reviewer Agent
...
### Delegate Output Validation

Before synthesizing findings, validate each delegate's output:

| Delegate | Expected Schema | Validation |
|----------|----------------|------------|
| python-code-reviewer | base-agent SUCCESS/FAILURE | Check `status` field |
| architecture-reviewer | base-agent SUCCESS/FAILURE | Check `status` field |
| test-executor | base-agent SUCCESS/FAILURE | Check `status` field |
| reliability-reviewer | reliability-reviewer output | Check `hats_completed` array |

**On Invalid Output**:
1. Log warning with raw output
2. Mark delegate section as INCOMPLETE
3. Continue with other delegates
4. Note in final report

4. **Apply Integration Checklist**
   - Load from skill: `integration-boundary-reviewer/reference/integration-checklist.md`
