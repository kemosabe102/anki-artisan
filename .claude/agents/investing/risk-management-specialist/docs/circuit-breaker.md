...
```python
loss_pct = (current_pnl / starting_equity) × 100
...
class TradingCircuitBreaker:
    ...
    def get_position_multiplier(self) -> float:
        return {NORMAL: 1.0, WARNING: 0.5, CRITICAL: 0.0, BREAKER: 0.0}[self.state]
```
...
- Time-based resets (allows gaming)
- Binary halt (no early warning)
- Manual override during session
- Counting only realized P&L (include unrealized)
