...
Test failure categorization enables automated agents to classify failures into actionable categories (TEST_BUG, APPLICATION_BUG, ENVIRONMENT) for intelligent delegation to appropriate fix agents. Using pytest result codes, exception patterns, stack trace analysis, and message heuristics, test-executor can automatically route failures without manual triage.
...
- When implementing categorization logic in test-executor
...
→ Step 5: Delegate → debugger (2), test-creator (1), orchestrator (1)
```
...
**Relationship**: test-executor categorizes APPLICATION_BUG failures and delegates to debugger

**Coordination Pattern**:

- test-executor runs tests → detects failures → categorizes as APPLICATION_BUG
- test-executor extracts failure details (file, function, error, stack trace)
- test-executor delegates to debugger with structured failure information
- debugger applies hypothesis-driven debugging → fixes app code
- debugger returns to test-executor for re-validation

**Example Usage**:

```python
# test-executor categorization and delegation
failures = parse_pytest_json_report("results.json")
for failure in failures:
    category, confidence = categorize_failure(failure)
...
- test-executor depends on debugger's fix quality for re-validation
- Both share failure detail schema (file, function, error, stack_trace)

---

### Integration 2: test-creator Agent

**Relationship**: test-executor categorizes TEST_BUG failures and delegates to test-creator

**Coordination Pattern**:

- test-executor runs tests → detects failures → categorizes as TEST_BUG
- test-executor identifies test redesign needs (fixture issues, assertion logic)
- test-executor delegates to test-creator with test improvement recommendations
- test-creator redesigns test code → returns improved tests
- test-executor re-runs to validate improvements
...
- test-creator depends on accurate TEST_BUG categorization
...
**Relationship**: test-executor categorizes ENVIRONMENT failures and escalates to orchestrator

**Coordination Pattern**:

- test-executor runs tests → detects failures → categorizes as ENVIRONMENT
- test-executor identifies infrastructure needs (missing deps, config)
- test-executor escalates to orchestrator with installation/configuration instructions
- orchestrator installs dependencies or fixes configuration
- orchestrator returns to test-executor for re-validation

**Example Usage**:

```python
# test-executor categorization and escalation
for failure in failures:
    category, confidence = categorize_failure(failure)
...
- test-executor depends on orchestrator's fix capability
...
- `.claude/docs/guides/test-executor/testing-flaky-detection.md`: Flaky test detection techniques
