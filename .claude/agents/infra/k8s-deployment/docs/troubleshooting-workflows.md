...
**Audience**: k8s-deployment agent (reference during error recovery and validation operations)
...
AGENT_NAME=k8s-deployment bash scripts/deployment/validate-k8s-manifests.sh \
...
**Domain Boundary**: k8s-deployment handles infrastructure deployment; delegate application instrumentation to python-code-implementer.
...
```bash
AGENT_NAME=k8s-deployment kubectl get configmap <name> -o yaml > configmap-current.yaml
```
...
**Cleanup old secrets**:
```bash
AGENT_NAME=k8s-deployment kubectl delete secret <old-secret-name> -n <namespace>
```
...
```bash
AGENT_NAME=k8s-deployment kubectl rollout history deployment/<name> -n <namespace>
```

#### 2. Identify Target Revision

```bash
AGENT_NAME=k8s-deployment kubectl rollout history deployment/<name> \
...
AGENT_NAME=k8s-deployment kubectl rollout undo deployment/<name> -n <namespace>

# Rollback to specific revision
AGENT_NAME=k8s-deployment kubectl rollout undo deployment/<name> \
...
AGENT_NAME=k8s-deployment kubectl describe deployment/<name> -n <namespace>
```
