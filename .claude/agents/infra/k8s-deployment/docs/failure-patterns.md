...
**Audience**: k8s-deployment agent (reference during troubleshooting operations)
...
1. **Get Events**:
   ```bash
   AGENT_NAME=k8s-deployment kubectl get events --sort-by=.lastTimestamp | grep <pod-name>
   ```

2. **Retrieve Logs** (use `--previous` for crashed container):
   ```bash
   AGENT_NAME=k8s-deployment kubectl logs <pod-name> --previous
   ```
...
**Investigation**:
```bash
AGENT_NAME=k8s-deployment kubectl describe pod <pod-name>
```
...
AGENT_NAME=k8s-deployment kubectl top pod <pod-name>
