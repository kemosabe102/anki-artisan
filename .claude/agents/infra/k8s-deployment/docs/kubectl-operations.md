...
```bash
# List resources
AGENT_NAME=k8s-deployment kubectl get pods -n <namespace>  # 🔥 Daily - Every deployment/troubleshooting
AGENT_NAME=k8s-deployment kubectl get deployments -n <namespace>  # 🔥 Daily - Every deployment check
AGENT_NAME=k8s-deployment kubectl get services -n <namespace>  # 🔥 Daily - Every deployment verification
AGENT_NAME=k8s-deployment kubectl get configmaps -n <namespace>  # ⚡ Weekly - Occasional config checks
AGENT_NAME=k8s-deployment kubectl get secrets -n <namespace>  # ⚡ Weekly - Occasional secret checks

# Detailed resource inspection
AGENT_NAME=k8s-deployment kubectl describe pod <pod-name> -n <namespace>  # 🔥 Daily - Every troubleshooting session
AGENT_NAME=k8s-deployment kubectl describe deployment <deployment-name> -n <namespace>  # ⚡ Weekly - Deep dive investigations

# Container logs
AGENT_NAME=k8s-deployment kubectl logs <pod-name> -n <namespace>  # 🔥 Daily - Every troubleshooting session
AGENT_NAME=k8s-deployment kubectl logs <pod-name> --previous -n <namespace>  # 🔥 Daily - CrashLoopBackOff diagnosis
AGENT_NAME=k8s-deployment kubectl logs <pod-name> -c <container-name> -n <namespace>  # ⚡ Weekly - Multi-container troubleshooting

# Event stream
AGENT_NAME=k8s-deployment kubectl get events --sort-by=.lastTimestamp -n <namespace>  # 🔥 Daily - Every troubleshooting session
AGENT_NAME=k8s-deployment kubectl get events --field-selector involvedObject.name=<pod-name> -n <namespace>  # 🔥 Daily - Pod-specific diagnosis

# API schema documentation
AGENT_NAME=k8s-deployment kubectl explain pod.spec.containers  # 📅 Monthly - Documentation lookup
AGENT_NAME=k8s-deployment kubectl explain deployment.spec.strategy  # 📅 Monthly - Documentation lookup

# Context management (read-only)
AGENT_NAME=k8s-deployment kubectl config current-context  # ⚡ Weekly - Context verification
AGENT_NAME=k8s-deployment kubectl config view  # 📅 Monthly - Configuration review
```
...
AGENT_NAME=k8s-deployment kubectl rollout status deployment/<name> -n <namespace>  # 🔥 Daily - After every deployment

# Trigger pod restarts (for ConfigMap/Secret updates)
AGENT_NAME=k8s-deployment kubectl rollout restart deployment/<name> -n <namespace>  # ⚡ Weekly - ConfigMap/Secret updates

# Rollback to previous revision
AGENT_NAME=k8s-deployment kubectl rollout undo deployment/<name> -n <namespace>  # 📅 Monthly - Emergency rollbacks
AGENT_NAME=k8s-deployment kubectl rollout undo deployment/<name> --to-revision=<N> -n <namespace>  # 📅 Monthly - Specific revision rollback

# View rollout history
AGENT_NAME=k8s-deployment kubectl rollout history deployment/<name> -n <namespace>  # ⚡ Weekly - Deployment audits
```
...
```bash
AGENT_NAME=k8s-deployment bash scripts/deployment/deploy-local-k8s.sh --cleanup
```
...
```bash
AGENT_NAME=k8s-deployment bash scripts/deployment/deploy-local-k8s.sh --validate-only
```
...
```bash
AGENT_NAME=k8s-deployment bash scripts/deployment/deploy-local-k8s.sh --logs
```
...
```bash
AGENT_NAME=k8s-deployment bash scripts/deployment/setup-k8s-secrets.sh --dry-run
```
...
3. **Verify deployment** (if needed):
   ```bash
   AGENT_NAME=k8s-deployment kubectl get pods -n <namespace>
...
4. **Restart pods** (if ConfigMap/Secret changed):
   ```bash
   AGENT_NAME=k8s-deployment kubectl rollout restart deployment/<name> -n <namespace>
   ```
...
```bash
if AGENT_NAME=k8s-deployment bash scripts/deployment/deploy-local-k8s.sh; then
    echo "Deployment succeeded"
else
    echo "Deployment failed - check logs"
    AGENT_NAME=k8s-deployment kubectl get events --sort-by=.lastTimestamp
fi
```
